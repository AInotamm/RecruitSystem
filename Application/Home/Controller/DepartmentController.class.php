...
class DepartmentController extends BaseController{
	...
	private function _departmentVal(){
		...
		for($i = 0;$i<$num-1;$i++){
			$depa_id = $depa[$i]['id'];
			$member[$i] = $this->_users->join('usersorganisation ON users.id = usersorganisation.users_id')->where("usersorganisation.department_id = '$depa_id'")->select();
		}
		//***********************************
	...
	public function addMem(){
		...
		$user_or = $this->_users_organisation->where($condtion4)->find();
		...
		if($stu_role > 5 || session('user_role') < $stu_role){
			...
			if(!$user_or){
				$this->_users_organisation->create($content2);
			}else{
				$this->_users_organisation->where($condition6)->save($content2);
			}
			if(!$stu_role){
			...
			if(session('user_role') <= I('post.position')){
				...
				if(!$user_or){
					$this->_users_organisation->add($content2);
				}else{
					$this->_users_organisation->where($condition6)->save($content2);
				}
				if(!$stu_role){
	...
	public function deletePre(){
		...
		if(session('user_role') > 5 ){
			...
			$content['role_id'] = '0';
			$this->_user_role->where($condtion)->save($content);
	...
	public function addContent(){
		$department_into = M('department_intro');
